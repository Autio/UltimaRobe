...
values=read_env(root/'.env') if (root/'.env').exists() else {}
bundled=values.get('BUNDLED_LOGIN')=='true'
check=subprocess.run([sys.executable,str(root/'scripts/doctor.py')]+([] if bundled else ['--network']),cwd=root)
if check.returncode:raise SystemExit(check.returncode)
command=['docker','compose','-f','compose.local.json' if bundled else 'compose.json']
