...
export function RealisticPreview({items,skin,hair,userId,corrections={},storageSuffix='',appearanceRevision=''}:{storageSuffix?:string;appearanceRevision?:string;items:Item[];skin:string;hair:string;userId:string;corrections?:Record<string,string>}){
 ...
 useEffect(()=>{let stopped=false;async function check(){try{const response=await fetch('/api/inventory/render?status=1');const result=await response.json();if(!stopped)setRendererAvailable(response.ok&&result.available===true);}catch{if(!stopped)setRendererAvailable(false);}}void check();const timer=setInterval(check,30000);return()=>{stopped=true;clearInterval(timer);};},[]);
 ...
 return <div className="ur-realistic">
  {rendererAvailable===false&&<p role="status">Realistic rendering is not connected. Your pixel wardrobe is ready to use; ask the person hosting this wardrobe to enable the optional renderer.</p>}
  ...
  <button className="ur-save-button" disabled={rendererAvailable!==true||!items.length||active||submitting||!measurementsLoaded||!!measurementError} onClick={generate}>{rendererAvailable===null?'Checking renderer…':rendererAvailable===false?'Renderer not connected':submitting?'Starting…':active?'Rendering locally…':job?'Generate new estimate':'Generate realistic estimate'}</button>
