...
export type SpriteJob={candidate?:SpriteJob;job_id:string;item_id:string;status:string;message:string;updated_at?:number;user_hint?:{correction?:string;cut_details?:string;primary_color?:string;pattern?:string};metadata?:{version?:string;cut_details?:string;primary_color?:string;pattern?:string}};
export function useSprites(userId?:string){
 ...
 useEffect(()=>{setJobs({});setCorrections({});pending.current.clear();if(!userId)return;
  ...
  void refresh();window.addEventListener('ultimarobe:sprites',refresh);const timer=setInterval(refresh,5000);return()=>{clearInterval(timer);window.removeEventListener('ultimarobe:sprites',refresh);};
 },[userId,refresh]);
